"""EU Dual-Use Regulation scraper."""
...
class EURegulationScraper(BaseScraper):
    """Scrapes EU Dual-Use Regulation pages."""

    def __init__(self, config: dict | None = None):
        super().__init__(config)
        cfg = config or load_config()
        country_cfg = cfg.get("scrapers", {}).get("countries", {}).get("eu", {})
        self.base_url: str = country_cfg.get("base_url", "https://policy.trade.ec.europa.eu")
        self.main_path: str = country_cfg.get("endpoints", {}).get(
            "main",
            "/development-and-sustainability/sustainable-trade/export-control-dual-use-items_en",
        )
    ...
    def _fetch_page_text(self, path: str) -> str:
        ...
        try:
            ...
            logger.warning("Failed to fetch EU regulation page %s: %s", url, exc)
            return ""

    def scrape(self, query: str) -> list[dict]:
        """Search EU page for query term and return matching context snippets."""
        ...
        logger.info("EU: %d result(s) for '%s'", len(results), query)
        return results
