...
class SentenceSimilarityEvaluation:
    ...
    def check_duplicate(self):
        ...
        for index, row in self.df.iterrows():
            if index == 10:  # Check only first 10 samples
                break
            q1 = row['question1']
            q2 = row['question2']
            ...
            # Ensure arrays are float32 for compatibility
            q1_array = np.array(q1_array, dtype='float32')
            q2_array = np.array(q2_array, dtype='float32')
...
if __name__ == "__main__":
    ...
    senteval = SentenceSimilarityEvaluation(zipcsvfile, 'sentence-transformer')  # or 'groq', 'tfidf', etc.
